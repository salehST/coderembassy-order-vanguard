...
export default function Dashboard({
  ...
  const maxActivity = Math.max(
    1,
    ...weeklySeries.map((day) => Number(day.count || 0)),
  );
    ...
    <div className="ceog-dashboard">
      ...
      {safeMode && (
        <div className="ceog-warning" role="status">
          ...
          <span>
            {__(
              "Order Vanguard Safe Mode is active - protection is monitoring only.",
              "coderembassy-order-vanguard",
            )}
          </span>
        </div>
      ...
      <section className="ceog-hero" aria-labelledby="ceog-hero-title">
        <div className="ceog-hero__copy">
          <div className="ceog-hero__badges">
            <span className="ceog-badge ceog-badge--light">
              {__("WooCommerce protection", "coderembassy-order-vanguard")}
            </span>
          ...
          <h2 id="ceog-hero-title">
            {__("CoderEmbassy Order Vanguard", "coderembassy-order-vanguard")}
          </h2>
          <p>
            {__(
              "API-level protection against card testing, bot orders, and fake checkouts.",
              "coderembassy-order-vanguard",
            )}
          </p>
          <button
            type="button"
            className="ceog-button ceog-button--hero"
      ...
      <section
        className="ceog-stat-grid"
        aria-label={__("Protection summary", "coderembassy-order-vanguard")}
      >
        ...
        <div
          className={
            "ceog-stat " +
            (breaker.remaining > 0 ? "ceog-stat--danger" : "ceog-stat--blue")
          }
        >
          <Clock3 size={19} aria-hidden="true" />
          <strong>
            {breaker.remaining > 0
              ? formatRemaining(breaker.remaining)
              : __("Ready", "coderembassy-order-vanguard")}
          </strong>
          <span>
            {breaker.remaining > 0 && enforcing
              ? __("Checkout paused", "coderembassy-order-vanguard")
              : __("Circuit breakers", "coderembassy-order-vanguard")}
          </span>
        </div>
      </section>
      ...
      <section className="ceog-quickstart">
        <div>
          <span className="ceog-section-kicker">
            {__("Monitor first", "coderembassy-order-vanguard")}
          </span>
          <h2>
            {__("Start with a clear baseline", "coderembassy-order-vanguard")}
          </h2>
          <p>
            {__(
              "Keep Monitor mode on while Order Vanguard records traffic. Switch to Enforce after the activity log matches what you expect.",
              "coderembassy-order-vanguard",
            )}
          </p>
        </div>
      ...
      <div className="ceog-dashboard-grid">
        <section
          className="ceog-dashboard-panel"
          aria-labelledby="ceog-weekly-title"
        >
          <div className="ceog-dashboard-panel__header">
            <div>
              <BarChart3 size={19} aria-hidden="true" />
              <h2 id="ceog-weekly-title">
                {__("Threat activity this week", "coderembassy-order-vanguard")}
              </h2>
            </div>
          ...
          <div className="ceog-weekly-chart">
            {weeklySeries.map((day) => (
              <div className="ceog-weekly-chart__day" key={day.date}>
                <span className="ceog-weekly-chart__value">
                    ...
                    style={{
                      height:
                        Number(day.count) > 0
                          ? Math.max(
                              8,
                              (Number(day.count) / maxActivity) * 100,
                            ) + "%"
                          : "0%",
                    }}
                  />
        ...
        <section
          className="ceog-dashboard-panel"
          aria-labelledby="ceog-recent-title"
        >
          <div className="ceog-dashboard-panel__header">
            <div>
              <Activity size={19} aria-hidden="true" />
              <h2 id="ceog-recent-title">
                {__("Recent protection events", "coderembassy-order-vanguard")}
              </h2>
            </div>
            <button
              type="button"
              className="ceog-button ceog-button--quiet"
              onClick={() => navigateTo("activity-log")}
            >
          ...
          {recentEvents.length ? (
            <ul className="ceog-recent-events">
              {recentEvents.map((event) => (
                <li key={event.id}>
                  <span
                    className={
                      "ceog-event-dot ceog-event-dot--" +
                      (event.mode === "enforce" ? "enforce" : "monitor")
                    }
                  />
                  <div>
                  ...
                  {event.order_url && (
                    <a
                      href={event.order_url}
                      aria-label={__(
                        "Open order",
                        "coderembassy-order-vanguard",
                      )}
                      title={__("Open order", "coderembassy-order-vanguard")}
                    >
            ...
            <div className="ceog-dashboard-empty">
              ...
              <span>
                {__("No protection events yet", "coderembassy-order-vanguard")}
              </span>
            </div>
          )}
        </section>
