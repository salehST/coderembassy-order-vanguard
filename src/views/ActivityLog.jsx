...
import { deleteLog, fetchLog, friendlyError } from "../api";
    ...
    if (
      ...
      !window.confirm(
        sprintf(
          ...
          __(
            "Permanently delete %d selected event(s)?",
    ...
    <div className="ceog-page ceog-log-page">
      ...
      <form className="ceog-log-filters" onSubmit={applyFilters}>
        <label className="ceog-field" htmlFor="ceog-log-type">
          ...
          >
            <option value="">
              {__("All events", "coderembassy-order-vanguard")}
            </option>
      ...
      {filters.ip_hash && (
        <div className="ceog-correlation-filter" role="status">
          ...
          <span>
            {__(
              "Showing events from the same attacker",
              "coderembassy-order-vanguard",
            )}
          </span>
      ...
      <section className="ceog-log-panel" aria-busy={loading}>
        ...
        {loading && result.rows.length < 1 ? (
          ...
          <div className="ceog-log-empty">
            ...
            <p>
              {__(
                "New protection events will appear here. Adjust the filters if you expected existing rows.",
                "coderembassy-order-vanguard",
              )}
            </p>
          </div>
                ...
                {result.rows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>
                      <span
                        className={
                          "ceog-mode-badge ceog-mode-badge--" + row.mode
                        }
                      >
                        {row.mode === "enforce"
                    ...
                    <td className="ceog-log-table__route">
                      {row.route || "-"}
                    </td>
                    <td className="ceog-log-table__reason">
                      {row.reason || "-"}
                    </td>
                    <td>
