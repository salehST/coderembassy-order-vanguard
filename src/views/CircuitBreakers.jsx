...
const makeDraft = (settings = {}) => ({
  ...
  breaker_global_cooldown: String(settings.breaker_global_cooldown || 120),
});
  ...
  if (!tier?.enabled) {
    return (
      <div className="ceog-breaker-live ceog-breaker-live--muted">
        ...
        <small>
          {__("Protection inactive", "coderembassy-order-vanguard")}
        </small>
      </div>
    );
  }
...
function TierPanel({ name, draft, status, onUpdate }) {
  ...
  return (
    <section
      className={
        "ceog-settings-panel ceog-breaker-panel ceog-breaker-panel--" + name
      }
    >
      ...
      <div className="ceog-breaker-fields">
        <label className="ceog-field" htmlFor={"ceog-" + name + "-threshold"}>
          ...
          <input
            ...
            onChange={(event) => onUpdate(config.threshold, event.target.value)}
            disabled={!enabled}
          />
        </label>
        <label className="ceog-field" htmlFor={"ceog-" + name + "-window"}>
          ...
          <input
            ...
            onChange={(event) => onUpdate(config.window, event.target.value)}
            disabled={!enabled}
          />
        </label>
        <label className="ceog-field" htmlFor={"ceog-" + name + "-duration"}>
          ...
          <input
            ...
            onChange={(event) => onUpdate(config.duration, event.target.value)}
            disabled={!enabled}
          />
        </label>
      </div>
          ...
          Object.entries(current.tiers).map(([key, tier]) => {
            ...
            return [
              key,
              {
                ...
                activeEntities: remaining > 0 ? tier.activeEntities : 0,
              },
            ];
          }),
        );
      ...
      <div className="ceog-save-bar">
        {saved && (
          <span className="ceog-saved" role="status">
            ...
            {__(
              "Circuit breaker settings saved",
              "coderembassy-order-vanguard",
            )}
          </span>
        )}
